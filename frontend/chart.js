  ...
  useEffect(() => {
    ...
    const generateMockData = () => {
      ...
      for (let i = 0; i < numPoints; i++) {
        ...
        if (rand < 0.6) {
          // Cluster 1: Early, low price
          time = d3.randomNormal(20, 10)();
          price = d3.randomNormal(3, 1)();
        } else if (rand < 0.9) {
          // Cluster 2: Mid-time, rising price
    ...
    const opacityScale = d3
      .scaleLinear()
      .domain(d3.extent(dataset, (d) => d.stake))
      .range([0.2, 1.0]);
    ...
    svg
      ...
      .append('line')
      .attr('x1', (d) => x(d.time))
      .attr('y1', (d) => y(d.price - 0.2))
      .attr('x2', (d) => x(d.time))
      .attr('y2', (d) => y(d.price + 0.2))
      .attr('stroke', '#ef4444')
      ...
      .on('mouseover', (event, d) => {
        ...
        tooltip
          .html(
            `Price: $${d.price.toFixed(2)}<br>Time: ${d.time.toFixed(1)}h<br>Stake: ${d.stake.toFixed(0)}`
          )
  ...
  return (
    <div className="container mx-auto p-4 lg:p-8 bg-white rounded-2xl shadow-lg max-w-5xl">
      ...
      <p className="text-center text-slate-500 mb-6">
        Green highlight shows high-confidence areas (&gt;75% cumulative probability)
      </p>

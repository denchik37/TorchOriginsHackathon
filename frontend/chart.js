...
const KdePriceChart = ({ data = defaultData }) => {
    ...
    useEffect(() => {
        ...
        // --- FIX: Add more padding to the domain to "zoom out" ---
        ...
        svg.append("g").attr("transform", `translate(0,${height})`).call(d3.axisBottom(x).ticks(width / 80).tickSizeOuter(0)).attr("class", "text-slate-500");
        svg.append("g").call(d3.axisLeft(y).ticks(height / 40).tickFormat(d3.format("$.3f"))).attr("class", "text-slate-500");

        svg.append("text").attr("text-anchor", "middle").attr("x", width / 2).attr("y", height + margin.bottom - 15).text("Time (hours from now)").attr("class", "text-sm font-medium text-slate-600");
        ...
        chartArea.append("g").selectAll("path").data(densityData).enter().append("path").attr("d", d3.geoPath()).attr("fill", d => densityColor(d.value)).attr("fill-opacity", 0.8);
        ...
        chartArea.append("g").selectAll("line").data(dataset).enter().append("line").attr("x1", d => x(d.time)).attr("y1", d => y(d.price - 0.0002)).attr("x2", d => x(d.time)).attr("y2", d => y(d.price + 0.0002)).attr("stroke", "#f43f5e").attr("stroke-width", 1.5).attr("stroke-opacity", d => opacityScale(d.stake))
            .on("mouseover", (event, d) => {
                tooltip.style("opacity", 1).html(`Price: $${d.price.toFixed(4)}<br>Time: ${d.time.toFixed(1)}h<br>Stake: ${d.stake.toFixed(0)}`).style("left", `${event.pageX + 15}px`).style("top", `${event.pageY - 15}px`);
            })
        ...
        function mousemove(event) {
            ...
            metricsHtml += `<div class="grid grid-cols-[auto,1fr] gap-x-2 gap-y-1">
                <span class="font-medium text-slate-500">Time:</span> <span class="text-right">${timeVal.toFixed(1)}h</span>
            ...
            if (nearbyPoints.length > 0) {
                 ...
                 <div class="grid grid-cols-[auto,1fr] gap-x-2 gap-y-1">
                    ...
                    <span class="font-medium text-slate-500">Avg Stake:</span> <span class="text-right">${d3.mean(nearbyPoints, d => d.stake).toFixed(0)}</span>
                 </div>`;
            }
...
import betData from '@/data/mock_bet_data.json'; // Assuming you create a /data folder
